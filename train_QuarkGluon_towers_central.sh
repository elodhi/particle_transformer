...
weaver \
    ...
    --data-fraction 0.08 \
    --data-config data/QuarkGluon/qg_${FEATURE_TYPE}_towers_central.yaml --network-config $modelopts \
    --model-prefix training/QuarkGluon_h5/${model}/{auto}${suffix}/net \
    --train-val-split 0.8 \
    --batch-size 300 --num-epochs 5 --gpus 0,1 \
    --start-lr $lr --optimizer ranger --log logs/QuarkGluon_h5_towers_${model}_{auto}${suffix}.log --predict-output pred_towers_central.root \
